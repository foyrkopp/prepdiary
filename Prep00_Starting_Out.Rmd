...
> 
> This was the status quo throughout virtually all of known history - unitl one of those subjugated people invented a weapon that can slay gods.
> 
> One extremely ruthless liberation war and lots of fallen deities later, the former vasall states are flourishing, while the former empire is a failed state under foreign occupation by the very people they once subjugated themselves.
> 
> Even beyond the devastations and population losses by war, virtually all of their infrastructure, once powered by miracles, has broken down. Divine artifacts have stopped working, been confiscated or became, bereft of all safeguards, too hazarduous to utilize.

There were more details on my mind, but I'll bring them up when they're relevant - for now, this should suffice.

The idea is that most of the campaign will take place in the occupied territories of the fallen empire, let's call them the Broken Lands for now. Lots of strife and limited efficacy of government means there'll be lots of space for individual heroism, evil warlords rising to power, magical artifacts waiting to be found and political powderkegs to be lit, defused or leveraged.
...
(To many tables, this might seem like an irrelevant problem, but I know from experience that my players care about such small implausibilites - and this is a problem that seems fixable.)

In short, I want a veneer of legitimacy for our party. Something that'd tell our hyptothetical mayor immediately that they can expect those budget Avengers to behave somewhat civilized, maybe even professional. And I don't want to hinge said legitimacy on a single PC who might have a respected background.
...
In particular, my players need an elevator pitch for the campaign so that they can either Yay or Nay it and think about how either their existing charactes might fit in or what new character they might play.
...
> 
> For (very long), the dominating continental superpower was a theocracy that owed it's supremacy to it's deities who not only interevened directly and personally in the world, but also granted fantastical supernatural powers to their followers - a high fantasy church-state, that conquered and exploited their neighboring countries with superior divine magic.
> 
> While those vasall states did not have equivalent divine support, they were much more technologically advanced.
> 
> Twenty years ago, the most advanced vasall state (a fantasy version of the terran ~17th century, including "normal" magic) found a way to kill gods.
> 
...
> 
> This war ended fiteen years ago.
> 
> While the liberated countries are busying themselves with a cultural and technological rennaissance, rather than working through the horrors of the war, the former territory of the collapsed theocracy is now formally a colony administrated by a coalition of former vasall states - and de facto a failed state, whose future is currently rather unclear: The Broken Lands.
> 
> 
...
> 
> Thus, you'll often find yourself solving the "problem of the week" (and being paid for it), but there'll also be plenty promising opportunites to pursue on your own initiative, if you care to do so - and, with time, an overarching plot should emerge as well.
> 
...
> 
> Due to the aforementioned code of conduct, and because the guild is not directly associated with any faction of the war, members are awareded a baseline of respect in most regions and are often hired to solve special problems.
> 
...
> 
> (If any of you, i.e. Nessia, would rather be employed by a noble house instead of being an indepent guild member, this would be fine as well. I'd just like to avoid you being a band of heavily armed Vandals without any advocate.)
> 
> 
> **Short draft of mechanical boundaries, this is negotiable:**
> 
> I'd like to downgrade the group to level three. I know, I know, higher levels = more cool stuff - but if we want to play somewhat consistentlly in this scenario, I'd prefer it if you don't already start out as heavyweights and level three seems like a good compromise for this (compared to level 1).
> 
...
> 
> - For reasons of fairness, I'd keep the free level one feat the established PCs already have, but it's explictly possible to leave this one open and learn a fitting feat later, during actual gameplay.
> 
...
> 
> (From where I'm standing, you don't need to coordiniate for a "balanced" party. If we wind up with an all-Rogue or all-Cleric party, we'll find ways to make it work.)
...
## Unneccessary Step 6 - I ~~need~~ want a world map
...
![Culture Map](images/safia_cultures.png)
...
The algorithm  placed a dash of a people with a finnic namespace up north, who were part of the empire, that's interesting. Did they somehow preserve a cultural identity? If so, how? Or did just their naming traditions endure? Either way, I'm keeping them.

Then I just slot in stuff that seems fun and plausible. A germanic namebase for the coastal region where a good chunk of the former vasall states are situated (including the aforementioned peninsula) and a greek one for a large southern island, that should do it - no need to go overboard.
...
![Nation Map](images/safia_nations.png)
...
Making them all a big, happy family would be a bit... *Star Trek*, so I insert some drama - *this* smallish country over here is actually a freshly seceeded province of *that* state and *those* neighbours helped them along. *These* people here and *those* over there both want to colonize (and maybe annex) that island down *here*, which the algorithm decreed to be virtually unpopulated, for it's resources - stuff like that.
...
![The full map](images/safia_fullmap.png)
...
For now, at least. I'm confiedent I've got enough input swirling around in my mind that I can present the world as a living, breathing place, and improvise unplanned details as needed.
...
We have not yet defined the The Code yet, so, at least viewed from a distance, the deed needs to be rather unambiguously bad. Let's say a member of The Guild has established somethelves as a local warlord and taken up banditry, taking people's stuff by force etc.
...
Obviously, the party will want to know where the Iron Johann's raids are occuring.
...
There's still a bit too many empty spaces on the map, so I add a bog somewhere in the vicinity of the river and a mysterious-yet-dangerous place of interest, a devastated forest region of the no-one-who-went-there-came-back category I'm callin Worldsend, at the very west.
...
![Local Map](images/renemi_localmap.png)
...
*Total Prep Time before next session:* Six hours, fourty minutes.

### Optional Sidekick
My party's current PCs have no wilderness survival skill whatsoever. While this tells me that I shouldn't go overboard with wilderness content, travel *should* be perilious.
...
- a penciled sketch of local map
- a basic premis of what the first adventure's task is
...
- Kai plays **Kristoff** (Human Sorcerer)
- Nick plays **Nessia** (Tiefling Paladin)
- Brian plays **Brice** (Human Wizard)
- Susan plays **Suhee** (Human Rogue)

(All names changed, obviously - let's hope I'll be able to keep them straight.)

## House Rule Discussion
...
### Narrative Group Initiative

We've already used Group Initiative before, to smooth things over.

All I do is add a slight twist:

- If it is clear from the narrative, which group should go first (i.e. the party stumbling, weapons drawn, into the Guard's break room), the narrative decides which group goes first.
- Otherwise, the member with the highest average initiative of each group rolls intitiative for the whole group. If any group member has advantage on initiative rolls, they get to roll their own initiative once, too - if they can beat the first roll, their roll is used instead.
...
(This is stolen from [treantmonk](https://www.youtube.com/watch?v=PbsTKreJwsk))
...
Susan can't make it today, so there's just three players at the table.
...
Other than that, we nail down the in-world interpretation of everyone's background (Kristoff is a mercenary Guild veteran, Brice & Nessia are members of noble houses, but have joined the Guild recently; Everyone hails from the Broken Lands) and I give them the rundown on the quest:

- The Guild has apporached them, either via another member or via mail about unverified reports of a member going Rogue somewhere in the Castrix Province (name provided by Azgaar's Map Algorithm), *probably* somewhere around Renemi.
- Their job is to travel to Renemi and find out what exactly happened. If they can solve the problem themselves, there's a cash bonus, but when in doubt, they're supposd to report back.
- Other Guild members are being sent to other regions within the province.

With that (and any questions about it) done, we jump into them arriving in the city.

## Relevant Events

One of the first things Kai asks about is one I did not prep for: Does the Guild maintain Guild Houses in the cities it's operating in?
...
Not just one, but two of them inquire about hiring a reliable wilderness guide, so I direct them to an improvised semi-legal liquor den called the *Bucket*, where they manage to recruit the half-drunk grumpy Goblin.

Overall, the (improvised) findings that might be relevant for the future are:

- The Iron Johan took two supply/relief convois shortly behind Immelgard, one and three months ago (for the record, it's spring now)
- He's a boastful character, always trying to appear of larger station than he genuinely is - spending generously, trying to mingle with nobility etc.
- His last legitimate, Guild-powered job was a long-time employment as an advisor in Bluewater (on the other side of the map)
- Most of the veterans don't think much of their bethren-turned-bandits. They were soldiers tasked with *protecting* the civilian (imperial) populace, robbing them instead is dishonorable. Nevertheless, they're hesitant to be truly helpful, those *are* their former comrades, after all
- The veterans come from a unit commonly just called the "4th Regiment" and were a unit of mostly Warg Handlers / Skirmishers.
- Many of the surviving Wargs have escaped to the wilderness and are a major danger on the road

Overall, it was a rather routined start (my players have done things like this a hundred times), but it served well in allowing them to find their feet and get an idea of both the adventure and the setting.
...
(I'll add a link to the relevant Reddit thread for discussion once that's up.)